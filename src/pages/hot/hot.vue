...
const currMap = hotMap.find((v) => v.type === query.type)
uni.setNavigationBarTitle({ title: currMap!.title })

const coverPic = ref('')
const subTypes = ref<(SubTypeItem & { finish?: boolean })[]>()
const activeIndex = ref(0)
const getHotRecommendData = async () => {
  const res = await getHotRecommendAPI(currMap!.url, {
    page: import.meta.env ? 30 : 1,
    pageSize: 10,
  })
  coverPic.value = res.result.bannerPicture
